  ...
  <div class="switch-tab">
    <div role="tablist" :class="['switch-tab__menu', 'switch-tab__menu--'+tabActiveDirection]">
      <div v-for="(tab, index) in tabs" :key="index" class="switch-tab__menu-item">
        <a :href="'#switch-tab-'+tab.id+'-panel'" :id="'switch-tab-'+tab.id+'-btn'" role="tab" :aria-controls="'switch-tab-'+tab.id+'-panel'" :aria-selected="tab.id === tabActiveId" :class="['switch-tab__menu-btn', { 'is-active' : tab.id === tabActiveId }]" @click.prevent="tabChange(tab)">{{tab.label}}</a>
      </div>
    </div>
    <slot></slot>
  </div>
